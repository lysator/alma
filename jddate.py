...
def jd_to_ymd(jd):
    if jd >= 2361390:
        return jd_to_ymd_gregorian(jd)
    elif jd > 2342041 and jd < 2346426:
        return jd_to_ymd_swedish(jd)
    else:
        return jd_to_ymd_julian(jd)
...
def FromString(str):
    ...
    if m:
        newdate.SetYMD(
            string.atoi(m.group(1)), string.atoi(m.group(2)), string.atoi(m.group(3))
        )
        return newdate
