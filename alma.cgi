...
def handle_cgi():
    ...
    if (
        form.getfirst("vcal_preview") is not None
        or form.getfirst("vcal_generate") is not None
    ):
    ...
    if not print_format:
        ...
        if month_based:
            ...
            for m in range(1, 13):
                so.write(
                    ...
                    % (m, selected(m == month), alma.month_names[m])
                )
            so.write("</SELECT>\n")
        else:
            # Vecka
            so.write(
                'Vecka <INPUT TYPE="TEXT" NAME="week" VALUE="%d" SIZE="2" onChange="this.form.submit();">\n'
                % (week)
            )

        # År
        so.write(
            '<INPUT TYPE="TEXT" NAME="year" VALUE="%d" SIZE="4" onChange="this.form.submit();">\n'
            % (year)
        )
        ...
        for value, label in (
            ...
            so.write(
                ...
                % (value, selected(calendar_type == value), label)
            )
        so.write("</SELECT>\n")
...
def handle_vcal(form):
    ...
    for param, default, text in [
        (str(alma.MRED), True, "Visa viktiga röda dagar"),
        ...
        if preview:
            ...
            so.write(
                '<TR><TD><INPUT TYPE="CHECKBOX" NAME="vcal_%s" VALUE="yes" %s></TD><TD>%s</TD></TR>\n'
                % (param, checked, text)
            )
    ...
    if preview:
        ...
        so.write(
            '<INPUT TYPE="SUBMIT" NAME="vcal_preview" VALUE="Uppdatera förhandsvisning">\n'
        )
        so.write(
            '<INPUT TYPE="SUBMIT" NAME="vcal_generate" VALUE="Ladda ner vCalendar-fil">\n'
        )
        so.write("</FORM>\n")
    ...
    if preview:
        so.write(
            "<P>Förhandsvisning av information som exporteras till vCalendar-filen:\n"
        )
        so.write("<PRE>\n")
    else:
